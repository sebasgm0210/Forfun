...
function readCheckoutReadyReservationIds() {
  if (typeof window === "undefined") return new Set<string>();

  try {
    const raw = window.localStorage.getItem(CHECKOUT_READY_STORAGE_KEY);
...
function writeCheckoutReadyReservationIds(ids: Set<string>) {
  if (typeof window === "undefined") return;

  const values = Array.from(ids);
  window.localStorage.setItem(CHECKOUT_READY_STORAGE_KEY, JSON.stringify(values));
  window.dispatchEvent(
    new CustomEvent("casa-luna-checkout-ready-change", { detail: values }),
  );
}
...
export function RecepcionCheckinPage() {
  ...
  const requestedTab =
    ...
    requestedTabParam === "checkin"
      ...
      : requestedReservation
        ? "checkin"
        : null;
  const checkinFocusHandledRef = useRef<string | null>(null);
  const checkoutFocusHandledRef = useRef<string | null>(null);
  const {
    reservations,
    advances,
    creditAccounts,
  ...
  const [checkoutReadyStayIds, setCheckoutReadyStayIds] = useState<Set<string>>(
    () => readCheckoutReadyReservationIds(),
  );
  ...
  }, [requestedReservation, requestedTab]);
  ...
  const checkoutReadyStays = inHouse.filter((stay) =>
  ...
  const filteredCheckoutReady = useMemo(() => {
    const text = query.toLowerCase().trim();
    if (!text) return checkoutReadyStays;

    return checkoutReadyStays.filter((stay) =>
      [stay.code, stay.guestName, stay.dpi, stay.phone, stay.roomNumber]
        .join(" ")
        .toLowerCase()
        .includes(text),
    );
  }, [checkoutReadyStays, query]);
  ...
  const groupedCheckoutReady = useMemo(
    () => groupStaysByGuestAndDate(filteredCheckoutReady, "checkout"),
    [filteredCheckoutReady],
  );

  useEffect(() => {
    if (!requestedReservation || requestedTab !== "checkin") return;

    const focusKey = `checkin|${requestedReservation}`;
    if (checkinFocusHandledRef.current === focusKey) return;

    const requestedId = numericApiId(requestedReservation);
    const targetStay = arrivals.find(
      (stay) =>
        stay.id === requestedReservation ||
        (requestedId !== null && numericApiId(stay.id) === requestedId),
    );
    if (!targetStay) return;

    const targetGroup = groupedArrivals.find((group) =>
      group.stays.some((stay) => stay.id === targetStay.id),
    );
    const groupKey = targetGroup ? `checkin|${targetGroup.id}` : null;

    checkinFocusHandledRef.current = focusKey;
  ...
  }, [arrivals, groupedArrivals, requestedReservation, requestedTab]);

  useEffect(() => {
    if (!requestedReservation || requestedTab !== "checkout") return;

    const focusKey = `checkout|${requestedReservation}`;
    if (checkoutFocusHandledRef.current === focusKey) return;
    ...
    setCheckoutReadyStayIds((current) => {
      const next = new Set(current);
      next.add(targetStay.id);
      writeCheckoutReadyReservationIds(next);
      return next;
    });

    const targetGroup = groupedCheckoutReady.find((group) =>
      group.stays.some((stay) => stay.id === targetStay.id),
    );
    const groupKey = targetGroup ? `checkout|${targetGroup.id}` : null;

    checkoutFocusHandledRef.current = focusKey;
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="pagos-estadia" className="space-y-4">
          <Panel
            title="Estadías actuales"
            description="Consulta huéspedes en habitación, registra abonos durante la estadía y manda una habitación a check-out cuando el cliente esté listo para salir."
            action={
              <div className="relative w-full sm:w-80">
                <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
                <TextInput
                ...
                filteredInHouse.map((stay) => {
                  ...
                  return (
                    <article key={stay.id} className="rounded-3xl border bg-background p-4 shadow-sm">
                      ...
                      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-muted/20 p-3 text-sm">
                        <div>
                          <p className="font-semibold">Estado operativo</p>
                          <p className="text-xs text-muted-foreground">
                            La habitación permanece In house hasta que recepción la mande a check-out.
                          </p>
                        </div>
                        <Button
                          type="button"
                          size="sm"
                          className="gap-2 rounded-full"
                          variant={checkoutReadyStayIds.has(stay.id) ? "outline" : "default"}
                          disabled={checkoutReadyStayIds.has(stay.id)}
                          onClick={() => markStayReadyForCheckOut(stay)}
                        >
                          <LogOut className="size-4" />
                          {checkoutReadyStayIds.has(stay.id)
                            ? "Lista para check-out"
        ...
        <TabsContent value="checkout" className="space-y-4">
          <Panel
            title="Habitaciones listas para check-out"
            description="Solo aparecen las habitaciones que recepción marcó como listas para check-out desde Estadías actuales."
            action={
              <div className="relative w-full sm:w-80">
                <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
                <TextInput
          ...
          >
            <div className="space-y-3">
              {groupedCheckoutReady.length === 0 ? (
                <div className="rounded-2xl border border-dashed p-8 text-center text-sm text-muted-foreground">
                  No hay habitaciones marcadas como listas para check-out con ese filtro.
                </div>
              ) : (
                groupedCheckoutReady.map((group) => {
